...
public class ActivityScanShipper extends BaseActivity implements BarcodeReader.BarcodeReaderListener {

    ActivityScanShipperBinding mActivityScanShipperBinding;

    ScanShipperViewModel mScanShipperViewModel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mScanShipperViewModel = ViewModelProviders.of(this).get(ScanShipperViewModel.class);
        mScanShipperViewModel.setActivityAndService(this, mMyService);
    ...
    public void getShipperData(String strShipperID) {
        this.strShipperID = strShipperID;
        onBackPressed();
    }
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (mUtility.haveInternet()) {
                                    ...
                                    hashMap.put("batchId", strBatchID);
                                    hashMap.put("lineNo", strLineNo);

                                    if(!isInLine)
                                        mScanShipperViewModel.scanShipperQrCode(mPreferenceHelper.getUserId(), mPreferenceHelper.getAccessToken(), strQrCode, hashMap);
                                    else
                                        mScanShipperViewModel.scanShipperQrCodeInLine(mPreferenceHelper.getUserId(), mPreferenceHelper.getAccessToken(), strQrCode, hashMap);
                                } else {
    ...
    public void onBackPressed() {
        CustomModel.getInstance().changeShipperState(true, strShipperID);
        super.onBackPressed();
    }
